  ...
  <div>
    ...
    <div class="ser_con">
      ...
      <ul v-if="storearr.length>0" class="ul11">
        ...
        <li v-for="(res,index) in storearr" :key="index" class="lione" @click.stop="goshop(res)">
          ...
          <div style="font-size:12px;width:85%;border-bottom:1px solid grey;padding-bottom:10px;margin-bottom:20px;">
            ...
            <p style="line-height:35px;">
              <span>月售</span> {{res.recent_order_num}}
              <span>单</span>
            </p>
      ...
      <ul class="ser_ul" v-if="inphistory!=''&&storearr.length==0">
        ...
        <p style="width:100%;text-align:center;padding:10px 0;background:white;" @click="clear()">清空搜索历史</p>
      </ul>
      <Footer></Footer>
    </div>
  </div>
      ...
      serbtn() {
        ...
        if (this.input10 != "") {
          ...
          this.$http({
            ...
            params: {
              geohash: this.$store.state.guess_city_infor.latitude +
                "," +
