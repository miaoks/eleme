    ...
    <div class="spec_wrap" v-if="buyer_hide">
      <div class="specification_choose">
        ...
        <div class="spec">
          <p>规格</p>
          <p>
            <span v-for="(spec_foo,index_foo) in basket_hidden_data.specfoods" :key="index_foo" @click.stop="change_spec_color(index_foo,basket_hidden_data.item_id,spec_foo.food_id,basket_hidden_data.specfoods)" :class="{'change1':index_data==index_foo}" class="change2">{{spec_foo.specs_name}}</span>
          </p>
        </div>
        <div class="add_buyer">
    ...
    <div class="content_wrap">
      ...
      <div class="content_left">
        <div class="content_warpper">
          <ul v-for="(value,index) in foods_list" :key="index"  @click="change_color($event,index)">
            <li class="goods_list" :class="{'change':index==current}">
          ...
          <div class="scroll_set">
            <div v-for="(food,i) in foods_list" :key="i" class="top_wrap">
              <div>
                ...
                <!-- 内容展示 -->
                <ul class="goods_show" v-for="(detail,inde) in food.foods" :key="inde" @click="togooddetail(detail)">
                  <li>
                    <div class="intro_wrap">
                      ...
                      <div class="description_wrap">
                        ...
                        <p class="description_sales">月售{{detail.month_sales}}份 好评率{{detail.satisfy_rate}}%</p>
                        <p class="goods_activity">
                          <span :style="setColor(detail.activity)">{{detail.activity&&detail.activity.image_text}}</span>
                        </p>
                      </div>
                    </div>
                    <div class="price_wrap">
                      ...
                      <i class="el-icon-circle-plus add_mark" @click.stop="add_order_count(detail.item_id,detail.specfoods[0].food_id,detail.specfoods)"></i>
                      </span>
                    </div>
                  </li>
                </ul>
            ...
            <section v-for="(v,i) in value.food" :key="i">
              <li class="list_show" v-if="value[value.food[i].food_id]">
                <p class="list_name">
                  ...
                  <span v-if="value.food.length>1" class="list_name_specs">{{value.food[i].specs_name}}</span>
                </p>
                <p class="list_price">￥{{value.food[i].price}}</p>
                <p class="list_quantity">
                  <i class="el-icon-remove-outline" @click.stop="delete_list(key,value.food[i].food_id,ind)"></i>
                  <span>{{value[value.food[i].food_id]}}</span>
                  <i class="el-icon-circle-plus-outline" @click.stop="add_list(key,value.food[i].food_id,value.food)"></i>
                </p>
              </li>
            </section>
          </section>
      ...
      <div class="buyer">
        <div class="display_wrap">
          ...
          <div v-if="total_order_num" class="pay_for_order" ref="count" @click="buyall()">结算</div>
        </div>
      </div>
    </div>
  </div>
  ...
  export default {
            ...
            for (let i = 0; i < this.item_id_obj[key].food.length; i++) {
              if (this.item_id_obj[key].food[i].food_id == k) {
                let obj = {
                  attrs: [],
                  extra: {},
                  id: this.item_id_obj[key].food[i].food_id,
                  name: this.item_id_obj[key].food[i].name,
                  packing_fee: this.item_id_obj[key].food[i].packing_fee,
                  price: this.item_id_obj[key].food[i].price,
                  quantity: this.item_id_obj[key][k],
                  sku_id: this.item_id_obj[key].food[i].sku_id,
                  specs: this.item_id_obj[key].food[i].specs,
                  stock: this.item_id_obj[key].food[i].stock,
                }
                this.arr.push(obj)
                  console.log(this.item_id_obj[key][k])
              }
            }
          }
        }
    ...
    add_chart() {
      ...
      this.$http({
        ...
        data: {
          restaurant_id: this.$route.params.food_list[0].restaurant_id,
          geohash: this.$route.params.detail.longitude +
            "," +
        ...
        this.$router.push({name:"makeorder",params:{res:res.data}})
      });
    }
    }
  };
